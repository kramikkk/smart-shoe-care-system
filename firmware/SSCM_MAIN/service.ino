...
void startService(String shoeType, String service, String care,
  ...
  if (service == "cleaning") {
    ...
    // heaters and exhaust fan to maintain the 35–40°C target band.
    LOG("[SERVICE] Drying — fan + PTC on; temp loop cools if >" + String(DRYING_TEMP_HIGH) + "C");
 ...
 * Maintains chamber temperature in the DRYING_TEMP_LOW–DRYING_TEMP_HIGH band (35–40°C)
 * using a hysteresis controller:
 *   < DRYING_TEMP_LOW  → PTC heaters ON, exhaust OFF  (heat up)
 *   > DRYING_TEMP_HIGH → PTC heaters OFF, exhaust ON  (cool down by venting)
 *   In band            → no change                    (hysteresis prevents rapid relay cycling)
 *
...
void handleDryingTemperature() {
  ...
  if (currentTemperature < DRYING_TEMP_LOW) {
    // Chamber too cold — enable both PTC heaters; disable exhaust to retain heat.
    if (!dryingHeaterOn) {
    ...
    // Chamber too hot — disable PTC heaters; vent excess heat via exhaust fan.
    if (dryingHeaterOn) {
      LOG("[DRYING] Temp " + String(currentTemperature, 1) + "C > " +
    ...
    if (!dryingExhaustOn) {
      ...
      setRelay(2, true); // Bottom exhaust fan ON
      dryingExhaustOn = true;
    }
  }
  // Temperature within [DRYING_TEMP_LOW, DRYING_TEMP_HIGH]: hold current state (hysteresis band).
}
